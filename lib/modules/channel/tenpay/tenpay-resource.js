/**
 * lib/module/channel/tenpay/tenpay-resource.js
 */
...
const utils = require('./tenpay-utils');
...
class TenpayResource extends ChannelResource {
  constructor(client) {
    super(client);
  }
    ...
    for (let i = 0; i < parameters.length; ++i) {
      ...
      if (m && isEither == false) {
        ...
        if (commonData.hasOwnProperty(paramName)) {
          ...
          throw new Exception.BadChannelRequest({
            message: 'Tenpay: Missing mandatory parameter "' + paramName + '"'
          });
        }
      } else if (m && isEither) {
        ...
        if (isAssigned == false) {
          throw new Exception.BadChannelRequest({
            message: 'Tenpay: Missing either-or parameter "' + paramName + '"'
          });
        }
      } else {
        throw new Exception.BadChannelRequest({
          message: 'Tenpay: Invalid specification "' + param + '"'
        });
      }
    }
    return reqData;
  ...
  _handleResponse(response) {
    try {
      ...
      if (res.return_code == 'SUCCESS') {
        if (res.result_code == 'SUCCESS') {
          ...
          if (isVerified) {
            ...
            throw new Exception.ChannelApiError({
      ...
      throw new Exception.ChannelApiError({
        message: err.message || 'Invalid XML received from Tenpay API'
      });
    }
  }
