...
passport.use(new LocalStrategy(
  function(username, password, done) {
    co(function* wrap() {
      ...
      if (!result) {
        done(new FusepayError.ServerError({
          status: Constant.HTTP.INTERNAL_SERVER_ERROR.CODE,
          code: 'SYSTEM_ERROR',
          message: 'System error'
        }), null);
      } else if (result && result.status == Result.STATUS.FAIL) {
        done(new FusepayError.BadRequestError({
  ...
  function(payload, done) {
    co(function* wrap() {
      ...
      if (user) {
        ...
        done(new FusepayError.UnauthorizedError({
          status: Constant.HTTP.UNAUTHORIZED.CODE,
          code: 'UNAUTHORIZED',
