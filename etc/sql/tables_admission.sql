...
CREATE TABLE IF NOT EXISTS t_admission_application(
  ...
  sales_id VARCHAR(32),
  agent_id VARCHAR(32),
  merchant_case_id BIGINT NOT NULL DEFAULT 0,
  branch_case_id BIGINT NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS t_admission_merchant(
  id BIGINT NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS t_admission_merchant_details(
  ...
  merchant_name VARCHAR(128) NOT NULL DEFAULT '', -- 商户名称
  contact_name VARCHAR(60) NOT NULL DEFAULT '',   -- 联系人姓名
  contact_email VARCHAR(100) NOT NULL DEFAULT '', -- 联系人邮箱
  contact_mobile VARCHAR(20) NOT NULL DEFAULT '', -- 联系人手机

  category_code VARCHAR(4) NOT NULL DEFAULT '', -- 行业大类
  category_subcode VARCHAR(4) NOT NULL DEFAULT '', -- 行业子类
  business_description VARCHAR(255) NOT NULL DEFAULT '',  -- 经营信息描述
  business_permit_image VARCHAR(255), -- 特别经营许可证照片（地址）
  office_phone VARCHAR(20) NOT NULL DEFAULT '',      -- 办公电话/客服电话
  office_province VARCHAR(40) NOT NULL DEFAULT '',   -- 注册地省份
  office_city VARCHAR(40) NOT NULL DEFAULT '',       -- 注册地城市
  office_district VARCHAR(40) NOT NULL DEFAULT '',       -- 注册地区县
  office_region_id VARCHAR(6) NOT NULL DEFAULT '', -- 注册地行政区编码
  business_hours VARCHAR(100) NOT NULL DEFAULT '',
  website_url VARCHAR(100),               -- 网站地址

  licence_name VARCHAR(255) NOT NULL DEFAULT '', -- 工商注册名称
  licence_address VARCHAR(255) NOT NULL DEFAULT '', -- 工商注册地址
  licence_no VARCHAR(40) NOT NULL DEFAULT '', -- 营业执照编号
  licence_scope_of_business VARCHAR(2048) NOT NULL DEFAULT '', -- 经营范围
  licence_since INT NOT NULL DEFAULT 0, -- 成立时间
  licence_expire INT NOT NULL DEFAULT 0, -- 结束时间
  licence_image VARCHAR(255) NOT NULL DEFAULT '', -- 营业执照图片（地址）

  org_type VARCHAR(20) NOT NULL DEFAULT '', -- 组织机构类型
                                -- ENTERPIRSE - 企业,
                                -- INDIVIDUAL_BUSINESS - 个体工商户
                                -- INSTITUTION - 事业单位
                                -- CHARITY - 慈善机构
  org_cert_no VARCHAR(40) NOT NULL DEFAULT '', -- 组织机构代码证编码（三证合一同营业执照）
  org_cert_since INT NOT NULL DEFAULT 0, -- 代码证生效日期（三证合一同营业执照）
  org_cert_expire INT NOT NULL DEFAULT 0, -- 代码证失效日期（三证合一同营业执照）
  org_cert_image VARCHAR(255) NOT NULL DEFAULT '', -- 代码证图片（地址， 三证合一同营业执照）

  legal_type VARCHAR(40) NOT NULL DEFAULT '', -- 法人/经办人
                                    -- LEGAL_REP - 法人,
                                    -- LEGAL_AGENT - 经办人
  legal_name VARCHAR(100) NOT NULL DEFAULT '', -- 法人姓名
  legal_cert_type VARCHAR(40) NOT NULL DEFAULT '',  -- 法人证件类型
                                                    -- IDENTITY_CARD - 身份证,
                                                    -- PASSPORT - 护照
  legal_cert_no VARCHAR(40) NOT NULL DEFAULT '', -- 法人证件编号
  legal_cert_since INT NOT NULL DEFAULT 0, -- 法人证件生效日期
  legal_cert_expire INT NOT NULL DEFAULT 0, -- 法人证件失效日期
  legal_cert_image1 VARCHAR(255) NOT NULL DEFAULT '', -- 法人证件照正面（地址）
  legal_cert_image2 VARCHAR(255) NOT NULL DEFAULT '', -- 法人证件照背面（地址）

  bank_province VARCHAR(40) NOT NULL DEFAULT '',        -- 开户银行省份
  bank_city VARCHAR(40) NOT NULL DEFAULT '',            -- 开户银行城市
  bank_district VARCHAR(40) NOT NULL DEFAULT '',            -- 开户银行区县
  bank_region_code VARCHAR(4) NOT NULL DEFAULT '',      -- 银行地区代码
  bank_name VARCHAR(100) NOT NULL DEFAULT '',           -- 银行名称
  bank_branch_name VARCHAR(100) NOT NULL DEFAULT '',      -- 支行名称
  bank_branch_code VARCHAR(12),               -- 支行联行号
  bank_account_type VARCHAR(40),               -- 账户类型
  bank_account_name VARCHAR(255) NOT NULL DEFAULT '',     -- 开户名称
  bank_account_no VARCHAR(40) NOT NULL DEFAULT '',        -- 开户账号

  contract_no VARCHAR(32), -- 合同编号

  is_imported TINYINT(1) NOT NULL DEFAULT 0,
  import_remark VARCHAR(100),
  is_submitted TINYINT(1) NOT NULL DEFAULT 0,
  branch_submitted TINYINT(1) NOT NULL DEFAULT 0,
  ...
  payment_application TEXT, -- 支付申请表, JSON对象
                            ...
                            --   'QQ_SCAN': '0.6'
                            -- }
  bd_type VARCHAR(40) NOT NULL DEFAULT '',  -- 拓展类型
                                    -- BD_INITIATIVE -自主拓展,
                                    -- MERCHANT_INITIATIVE - 商户申请,
                                    -- BD_AGENT - 代理商拓展
  agent_id VARCHAR(28),    -- 代理商ID,
  agent_name VARCHAR(128),  -- 代理商名称
  bd_name VARCHAR(128), -- 拓展人员名称,
